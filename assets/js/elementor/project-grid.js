(function ($) {
    var RovokoWidgetProjectsHandler = function ($scope, $) {
        var $project_grid = $scope.find('.rovoko-project-wrapper').eq(0);
        if ($project_grid.length > 0) {
            ...
            $project_grid.find('.view-all-btn a.ef5-btn').live('click', function () {
                ...
                jQuery.get($link, function (data) {
                    ...
                    var $items = $(data).find('#' + $id).find('.projects').html();
                    $project_grid.find('.projects').append($items).show('slow');
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/rovoko_project_grid.default', RovokoWidgetProjectsHandler);
    });
})(jQuery);