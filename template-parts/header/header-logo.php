...
*/
// Default Logo
$logo             = rovoko_get_opts( 'logo', array( 'url' => '', 'id' => '' ) );
$logo_size        = rovoko_get_opts( 'logo_size', array( 'width' => rovoko_configs('logo_width'), 'height' => rovoko_configs('logo_height'), 'units' => rovoko_configs('logo_units') ) );

$logo_size_w      = rovoko_extract_numbers($logo_size['width']) ? rovoko_extract_numbers($logo_size['width']) : rovoko_configs('logo_width') ;
$logo_size_h      = rovoko_extract_numbers($logo_size['height']) ? rovoko_extract_numbers($logo_size['height']): rovoko_configs('logo_height');
...
if ( !empty($logo['id']) ) {
    ...
    if($logo_mime_type !== 'image/svg+xml'){
        $logo_url        =  rovoko_get_image_url_by_size( [
            'id'   => $logo['id'], 
            'size' => $logo_size_w.'x'.$logo_size_h
        ]);
        $logo_url_retina =  rovoko_get_image_url_by_size( [
            'id'   => $logo['id'], 
...
$ontop_logo             = rovoko_get_opts( 'ontop_logo', array( 'url' => '', 'id' => '' ) );
$ontop_logo_size        = rovoko_get_opts( 'ontop_logo_maxh', array( 'width' => rovoko_configs('logo_width'), 'height' => rovoko_configs('logo_height'), 'units' => rovoko_configs('logo_units') ) );
$ontop_logo_size_w      = rovoko_extract_numbers($ontop_logo_size['width']) ?  rovoko_extract_numbers($ontop_logo_size['width']) : rovoko_configs('logo_width');
$ontop_logo_size_h      = rovoko_extract_numbers($ontop_logo_size['height']) ? rovoko_extract_numbers($ontop_logo_size['height']) : rovoko_configs('logo_height');
$ontop_logo_size_retina = ($ontop_logo_size_w*2).'x'.($ontop_logo_size_h*2);
$ontop_logo_url         = get_template_directory_uri() . '/assets/images/logo/logo.png';
$ontop_logo_url_retina  = get_template_directory_uri() . '/assets/images/logo/logo-white.png';
if ( !empty($ontop_logo['id']) ) {
    $logo_mime_type  = get_post_mime_type($ontop_logo['id']);
    if($logo_mime_type !== 'image/svg+xml'){
        $ontop_logo_url        =  rovoko_get_image_url_by_size( [
            'id'   => $ontop_logo['id'], 
            'size' => $ontop_logo_size_w.'x'.$ontop_logo_size_h
        ]);
        $ontop_logo_url_retina = rovoko_get_image_url_by_size( [
            'id'   => $ontop_logo['id'], 
...
$sticky_logo             = rovoko_get_opts( 'sticky_logo', array( 'url' => '', 'id' => '' ) );

$sticky_logo_size        = rovoko_get_opts( 'sticky_logo_maxh', array( 'width' => rovoko_configs('logo_width'), 'height' => rovoko_configs('logo_height'), 'units' => rovoko_configs('logo_units') ) );
$sticky_logo_size_w      = rovoko_extract_numbers($sticky_logo_size['width']) ? rovoko_extract_numbers($sticky_logo_size['width']) : rovoko_configs('logo_width');
$sticky_logo_size_h      = rovoko_extract_numbers($sticky_logo_size['height']) ? rovoko_extract_numbers($sticky_logo_size['height']) : rovoko_configs('logo_height');
$sticky_logo_size_retina = ($sticky_logo_size_w*2).'x'.($sticky_logo_size_h*2);

$sticky_logo_url         = get_template_directory_uri() . '/assets/images/logo/logo.png';
$sticky_logo_url_retina  = get_template_directory_uri() . '/assets/images/logo/logo-white.png';
...
if ( !empty($sticky_logo['id'] ))
{
    ...
    if($logo_mime_type !== 'image/svg+xml'){
        $sticky_logo_url        = rovoko_get_image_url_by_size( [
            'id'   => $sticky_logo['id'], 
            'size' => $sticky_logo_size_w.'x'.$sticky_logo_size_h
        ]);
        $sticky_logo_url_retina    = rovoko_get_image_url_by_size( [
            'id'   => $sticky_logo['id'], 
