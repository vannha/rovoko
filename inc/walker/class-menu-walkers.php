<?php
/**
 * Rovoko_Main_Menu_Walker
 * Rovoko_Side_Main_Menu_Walker
 * Rovoko_Mega_Menu_Walker
 *
 * @package EF5 Theme
 *
 */
...
class Rovoko_Main_Menu_Walker extends Walker_Nav_Menu{
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
        $indent = str_repeat("\t", $depth);
        $header_layout = rovoko_get_opts('header_layout');
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if(empty($title)) $title = sprintf( __( '#%d (no title)','rovoko' ), $item->ID );
        /* add expander */
        $item_expander = '';
        $is_parent = in_array('menu-item-has-children', $classes);
        if($is_parent === true) $item_expander =  rovoko_widget_expander();
...
class Rovoko_Side_Main_Menu_Walker extends Walker_Nav_Menu{
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
        $indent = str_repeat("\t", $depth);
        $output .= "\n$indent<ul class=\"ef5-submenu ef5-toggle-menu ef5-side-submenu ef5-side-submenu-base\">\n";
    }
 ...
 * @return boolean
*/
if(!function_exists('rovoko_enable_megamenu')){
	add_filter('ef5_enable_megamenu', 'rovoko_enable_megamenu');
	function rovoko_enable_megamenu(){
	    return false;
	}
}
if(!function_exists('rovoko_megamenu_locations')){
    add_filter('ef5_locations','rovoko_megamenu_locations');
    function rovoko_megamenu_locations(){
