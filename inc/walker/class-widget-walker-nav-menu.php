...
class Rovoko_Menu_Walker extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if(empty($title)) $title = sprintf( esc_html__( '#%d (no title)','rovoko' ), $item->ID );
		$title = '<span class="title">'.$title.'</span>';
		/* add expander */
		$item_expander = $item_no_link_expander = '';
		if($item->url === '#') $item_no_link_expander = rovoko_widget_expander(['class'=>'ef5-toggle-overlay']);
		$is_parent = in_array('menu-item-has-children', $classes);
		if($is_parent === true) {
			$item_expander = rovoko_widget_expander();
		}
