 ...
 * get page ID by Slug
*/
function rovoko_get_id_by_slug($slug, $post_type){
...
function rovoko_get_link_by_slug($slug, $post_type = 'post'){
 ...
 * get content by slug
**/
function rovoko_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
...
**/
if(!function_exists('rovoko_content')){
    function rovoko_content($id){
...
**/
if(!function_exists('rovoko_content_by_slug')){
    function rovoko_content_by_slug($slug, $post_type){
        $content = rovoko_get_content_by_slug($slug, $post_type);
        $id = rovoko_get_id_by_slug($slug, $post_type);
        $shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
        if ( ! empty( $shortcodes_custom_css ) ) {
            $shortcodes_custom_css = strip_tags( $shortcodes_custom_css );
 ...
 *
*/
function rovoko_get_content_link( $args = []){
    $args = wp_parse_args($args, [
        'content' => '',
        'class'   => 'content-link btn btn-pri',
        'target'  => '_blank',
        'prefix'  => esc_html__('Visit','rovoko'),
        'echo'    => true
    ]);
    $link = $title = '';
 ...
 *
*/
function rovoko_get_content_image( $args = []){
    $args = wp_parse_args($args, [
        'content' => '',
        'class'   => 'content-image',
        'echo'    => true
 ...
 */
if ( ! function_exists( 'rovoko_get_attachment_id_from_url' ) ) {
    
    function rovoko_get_attachment_id_from_url( $url ) {
 ...
 * 
*/
function rovoko_extract_numbers($string,$pos=0)
{
 ...
 * @return ID
*/
function rovoko_get_id_by_title($post_title, $post_type = 'page'){
 ...
 * Output html
*/
if(!function_exists('rovoko_html')){
    function rovoko_html($html){
        return $html;
    }
}
 ...
 * @since 1.0.0
*/
if(!function_exists('rovoko_get_custom_post_taxonomies')){
    function rovoko_get_custom_post_taxonomies($post_type, $key)
    {
 ...
 * @since 1.0.0
*/
if(!function_exists('rovoko_get_custom_post_cat_taxonomy')){
    function rovoko_get_custom_post_cat_taxonomy()
    {
        $post = get_post();
...
*/
function rovoko_get_taxo_slug_as_css_class($args = [])
{
...
function rovoko_terms($args=[]){
    ...
    echo apply_filters('rovoko_terms', $terms_string);
}
 ...
 *
*/
function rovoko_get_term_id_by_slug($post_type, $taxo_key, $term_slugs){
    if(empty($term_slugs)) return;
    $term_slugs = explode(',', $term_slugs);
    $term_ids = [];
    foreach ($term_slugs as $slug) {
        $term = get_term_by('slug', $slug, rovoko_get_custom_post_taxonomies( $post_type , $taxo_key));
 ...
 *
*/
function rovoko_tax_query($post_type, $taxonomies, $taxonomies_exclude ){
    $tax_query = array();    
    if(!empty($taxonomies) || !empty($taxonomies_exclude)) {
        ...
        foreach ($terms as $term) {
            $real_terms_args = [
                'taxonomy' => rovoko_get_custom_post_taxonomies( $post_type , $term), 
                'exclude'  => rovoko_get_term_id_by_slug($post_type, $term, $taxonomies_exclude)
            ];
...
function rovoko_hex2rgba($color, $opacity = false) {
