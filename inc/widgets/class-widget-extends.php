...
class Rovoko_Widget_Extends
{
    ...
    function extend_widget_form( $widget, $return, $instance )
    {
        ...
        if ( is_a( $widget, 'WP_Widget_Tag_Cloud' ) || is_a($widget, 'WC_Widget_Product_Search') )
        {
        ...
        <p>
            ...
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-use_theme_style"><?php esc_html_e( 'Use Theme Style','rovoko' ); ?></label>
        </p>
        <?php
        }
        ...
        ?>
        <p>
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-el_class"><?php esc_html_e( 'CSS Class','rovoko' ); ?></label>
        ...
        <p>
            ...
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-hide_title"><?php esc_html_e( 'Hide widget title','rovoko' ); ?></label>
        </p>
        <?php
    ...
    function extend_widget_update( $instance, $new_instance, $old_instance, $object )
    {
        ...
        if ( is_a( $object, 'WP_Widget_Tag_Cloud' ) || is_a($object, 'WC_Widget_Product_Search') )
        {
    ...
    function extend_widget_params( $params )
    {
        ...
        if ( isset( $instance['el_class'] ) || is_a( $widget_obj['callback'][0], 'WP_Widget_Tag_Cloud' ) || is_a( $widget_obj['callback'][0], 'WC_Widget_Product_Search' ) )
        {
            $classes = array();
            ...
            if ( is_a( $widget_obj['callback'][0], 'WP_Widget_Tag_Cloud' ) || is_a( $widget_obj['callback'][0], 'WC_Widget_Product_Search' ) )
            {
