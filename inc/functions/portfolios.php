...
*/
if(!function_exists('rovoko_portfolio_info')){
	function rovoko_portfolio_info($args = []){
		if(!is_singular('ef5_portfolio')) return;
		$args = wp_parse_args($args, [
			'title' => esc_html__('Project Description','rovoko')
		]);
		// Client name
		$client = rovoko_get_post_format_value('portfolio_client_name','Magdalen School');
		// Location
		$location = rovoko_get_post_format_value('portfolio_location','Oslay, Canada');
		// Surface Area
		$surface_area = rovoko_get_post_format_value('portfolio_surface_area','3,000,000 m2');
		// Budget
		$budgets = rovoko_get_post_format_value('portfolio_budgets','$5,000,000.00');
		// Date
		$date = rovoko_get_post_format_value('portfolio_complete','');
		$date = strtotime($date);
	...
	?>
		<div class="pf-sidebar-box detail-box">
			<div class="ef5-heading h3"><?php echo rovoko_html($args['title']); ?></div>
			<div class="inner detail-inner ef5-box-shadow ef5-bg-white transition">
				<ul class="pf-details-list ef5-unstyled">
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Client','rovoko'); ?>:</strong>
						<?php echo esc_html($client); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Location','rovoko'); ?>:</strong>
						<?php echo esc_html($location); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Surface Area','rovoko'); ?>:</strong>
						<?php echo esc_html($surface_area); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Budgets','rovoko'); ?>:</strong>
						<?php echo esc_html($budgets); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Complete','rovoko'); ?>:</strong>
						<?php echo esc_html($date); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Categories','rovoko'); ?>:</strong>
...
*/
if(!function_exists('rovoko_portfolio_featured')){
	function rovoko_portfolio_featured($args = []){
		$args = wp_parse_args($args, [
			'title'          => esc_html__('Featured Projects','rovoko'),
			'posts_per_page' => '3'
		]);
            ...
            <div class="pf-sidebar-box pf-featured-box">
	        	...
	        	<div class="inner feature-post-inner ef5-box-shadow ef5-bg-white transition">
		            <?php while ( $r->have_posts() ) {
		                $r->the_post();
		                $thumbnail_url = rovoko_get_image_url_by_size( ['size' => implode('x', $thumbnail_size), 'default_thumb' => true,'class'=>'d-block'] );
		                echo '<div class="pf-feature-item ef5-box-shadow-hover-17 transition"><div class="row gutters-20 ">';
			                ...
			                echo '<div class="ef5-brief col" style="max-width: calc(100% - '.((int)$thumbnail_size[0]+20).'px);">';
				                ...
				                printf(
				                	...
				                	get_the_term_list( get_the_ID(), rovoko_get_post_taxonomies(), '', ', ', '' )
				                );
			                echo '</div>';
		                echo '</div></div>';
		            } ?>
 ...
 * @param $user_ID
*/
if(!function_exists('rovoko_portfolio_supporter')){
	function rovoko_portfolio_supporter($args = []){
		$args = wp_parse_args($args, [
			'email' => rovoko_get_post_format_value('post-support',''),
			'title' => esc_html__('Support','rovoko')
		]);
		if(empty($args['email']) || $args['email'] = null) return;
	        $user = get_user_by( 'email', rovoko_get_post_format_value('post-support','') );
	        if($user !== false){ 
	            global $wp_roles; 
 ...
 * @param $user_ID
*/
if(!function_exists('rovoko_portfolio_attachment')){
	function rovoko_portfolio_attachment($args = []){
		$args = wp_parse_args($args, [
			'title' => esc_html__('Download','rovoko'),
			'class' => ''
		]);
		$number_of_att = apply_filters('rovoko_number_of_portfolio_attachment', 5);
		$files = [];
		for ($i=0; $i <= $number_of_att ; $i++) { 
			$att = rovoko_get_post_format_value('post-attachment-'.$i,'');
			if(!empty($att)){
				$files[] = $att;
			}
		}
